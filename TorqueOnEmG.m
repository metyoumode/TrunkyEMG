...
for j=1:length(joints)
   ...
   torque=Log.logsout{17}.Values.(sprintf('PDO_M%d',joint)).(sprintf('torque_actual_%d',joint));
          ...
          %  time = allTxtData.(condition).(movement).time;
            ...
            for m = 1:numMuscles
                ...
                signal_norm_reduce=downsample(signal_norm,scalingFactor);
                    ...
                    plot(torque.Data(StartFrontFlex(c):EndFrontFlex(c)), signal_norm_reduce(1:(EndFrontFlex(c)-StartFrontFlex(c)+1),m), 'Color', "#A2142F");
                ...
                signal_norm_reduce=downsample(signal_norm,scalingFactor);
                    ...
                    % Plot RMS envelope normlized to MVC
                    plot(torque.Data(StartLatFlex(c):EndLatFlex(c)), signal_norm_reduce(1:(EndLatFlex(c)-StartLatFlex(c)+1),m), 'Color', "#A2142F");
            ...
            sgtitle(sprintf('Condition: %s, movement: %s Joint: %d', condition, movement,joint));
